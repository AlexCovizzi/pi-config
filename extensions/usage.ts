...
import { CancellableLoader, matchesKey, truncateToWidth, visibleWidth } from '@mariozechner/pi-tui'
...
function emptyBaseStats(): BaseStats {
  return { messages: 0, cost: 0, tokens: emptyTokens() }
}

function emptyModelStats(): ModelStats {
  return { ...emptyBaseStats(), sessions: new Set() }
}

function emptyProviderStats(): ProviderStats {
  return { ...emptyBaseStats(), sessions: new Set(), models: new Map() }
}

function emptyTimeFilteredStats(): TimeFilteredStats {
  return {
    providers: new Map(),
    totals: { sessions: 0, ...emptyBaseStats() },
  }
}
...
async function collectUsageData(signal?: AbortSignal): Promise<UsageData | null> {
  ...
  for (const filePath of sessionFiles) {
    ...
    for (const msg of messages) {
      ...
      const tokens = {
        // Total = input + output only (excludes cache to avoid double-counting).
        // cacheRead tokens were already counted as input when first sent, so
        // including them would inflate totals since cache hits repeat every message.
        // The `cache` field tracks cacheRead + cacheWrite for informational display.
      ...
      await ctx.ui.custom<void>((tui, theme, _kb, done) => {
        ...
        return {
          render: (w: number) => {
            ...
            return ['', ...usageLines, '', bottomBorder]
          },
          invalidate: () => {},
